...
//===-------------------------------------------------- --------===//
//
// VENDORED, NEARLY VERBATIM, as a control experiment for NpuEmbeddings.
// 35 of its 40 code lines are upstream aie_kernels/aie2p/gelu.cc unchanged.
//
// Changes made (Apache-2.0 section 4b):
//   1. the include path is "aie_kernel_utils.h" rather than
//      "../aie_kernel_utils.h", because this file is compiled from our own
//      ExternalFunction path with our own include dirs;
//   2. the entry point is renamed gelu_bf16 -> gelu_ctrl_bf16, so it can
//      coexist with our kernels in one build without a symbol collision.
//
// It exists to answer one question and no other: when our own GELU measured
// 1.33e-02 against the goldens, was the error AMD's kernel or our build of
// it? Running THEIR source through OUR toolchain separates those. The answer
// was that the error is aie::tanh itself -- see tasks/0014.
...
void gelu_ctrl_bf16(bfloat16 *restrict input, bfloat16 *restrict output) {
